...
def comparison(I,J,K,r,nb_rand,n_samples,n_samples_err,exact_err=False,scale=False,noise_level=0.1):
  """
  ...
  for i in range(nb_rand) : 
    ...
    for k in range(5):
      ...
      factors=random_init_fac(t,r)
      # run 4 methods
      weights1,factors1,it[1],error[1],cpt1,l_fac[1],time[1]=her_Als(t,r,factors=copy.deepcopy(factors),it_max=200,tol=1.1*noise_level,list_factors=True,time_rec=True)  
      weights2,factors2,it[2],error[2],l_fac[2],time[2]=als(t,r,factors=copy.deepcopy(factors),it_max=200,tol=1.1*noise_level,list_factors=True,time_rec=True) 
      weights3,factors3,it[3],error[3],err_es3,l_fac[3],time[3]=CPRAND(t,r,n_samples,n_samples_err,factors=copy.deepcopy(factors),exact_err=True,it_max=200,err_it_max=200,tol=1.1*noise_level,list_factors=True,time_rec=True)
      weights4,factors4,it[4],error[4],err_es4,cpt4,l_fac[4],time[4]=her_CPRAND(t,r,n_samples,n_samples_err,factors=copy.deepcopy(factors),exact_err=True,it_max=200,err_it_max=200,tol=1.1*noise_level,list_factors=True,time_rec=True) 
      # information storage
      restart["herals"].append(cpt1)
      restart["hercprand"].append(cpt4)
      for j in range(1,5):
        fit[j].append(1-(error[j][len(error[j])-1]))
        score_tot[j].append(score(fac_true,l_fac[j][len(l_fac[j])-1]))
        it_tot[j].append(it[j])
        time_tot[j].append(np.cumsum(time[j])[len(time[j])-1])
  # figure
  labels=["herals","als","cprand","hercprand"]
  ...
  plt.yticks(range(1, 3), ["herals","hercprand"])
  plt.title('restarts')
